...
\item The system should be distributed: we expect the simulator to manage many
  ...
  when the traffic load will increase greatly and the system must prove to be scalable
...
We'll show which concurrency issues we took into account during the initial phase of
the project.
...
We'll use figure \ref{fig:pa-sample-street} and figure
...
Thus, we'll have to find a solution to avoid rear-end collisions.
...
We'll use \href{http://www.aci.it/i-servizi/normative/codice-della-strada/titolo-v-norme-di-comportamento/art-145-precedenza.html}{Italian yield rules}\footnote{we'll also assume that vehicles proceed on the right side of the
street}, therefore if a vehicle's trajectory is met by another one from right,
the latter has to wait for the former to pass.

Looking at picture \ref{fig:pa-sample-crossroad}, D is free to go since there
is no vehicle on the right of its trajectory and so it is C.

B must wait D to pass in order to turn left, whereas it does not have to wait
for C to turn right because B's and C's trajectories do not intersect.

Finally, A.1 must wait for B to pass, while A.2 must wait for A.1, B and C.
