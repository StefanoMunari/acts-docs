...
\begin{itemize}
  ...
  \item \textit{Functional}: Elixir is a pure functional language so each
        ...
        Other alternatives (e.g., Scala) do not guarantee anything about
        immutable state if not explicitly defined so, therefore the state of an
        actor may change due to external factors when processing a message
        (!)
