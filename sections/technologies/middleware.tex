...
\begin{itemize}
  ...
  \item \textit{Actor model}: OTP fits perfectly in the actor model, since the
        ...
        example, in Scala an actor may suffer of concurrent
        modifications of its state \textit{even when it is processing a
        message};
...
\begin{itemize}
  ...
  \item \textit{GenStage}: from Elixir 1.4, Flow and the GenStage behaviour
        (i.e., the equivalent of an abstract class in OOP) has been iimplemented.
