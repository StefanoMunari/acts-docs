...
\begin{enumerate}
  \item A node N sends an \textit{election} message to each node M with \textit{higher} priority/ID than it;
...
\begin{itemize}
  \item If none of the M nodes reply then N elects itself as the coordinator sending a \textit{coordinator} message to each node L which has lower priority/ID than N;
  \item If the dead node is the highest priority/ID node H and it restarts during the election process then it sends a \textit{coordinator} message to each active node L in the system. Each node L possibly updates its coordinator status if the coordinator election isn't finished yet;
